...
export const RESOURCE_TIP_I18N = (t: (k: string) => string) => ({
...
export const saveResource = async (data, callback, emit) => {
  ...
  try {
    ...
    useNotify({ type: 'success', message: `${isEdit ? '修改' : '创建'}成功` })
    emit('refresh', state.type)
    state.refresh = true
    callback()
  } catch (error) {
    useNotify({ type: 'error', message: `工具类${isEdit ? '修改' : '创建'}失败：${error.message}` })
  }
}
