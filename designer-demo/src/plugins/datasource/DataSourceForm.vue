<template>
  ...
  >
    <template #header>
      <button-group>
        <tiny-button class="field-save" type="primary" @click="save">{{ t('designer.common.save') }}</tiny-button>
        ...
        requestDeleteDataSource(state.dataSource.id)
          .then((data) => {
            if (data) {
              ...
              useNotify({
                title: t('designer.datasource.dataSourceDeleteSuccess'),
                type: 'success'
              })
              close()
              emit('save')
          ...
          .catch((error) => {
            message({ message: `${t('designer.datasource.dataSourceDeleteFailed')},${error?.message || ''}`, status: 'error' })
          })
          ...
          settingRef.value.saveRecord().then((record) => {
            ...
            if (props.editable) {
              requestUpdateDataSource(state.dataSource.id, editRequestData).then(() => {
                ...
                useNotify({
                  title: t('designer.datasource.dataSourceModifySuccess'),
                  type: 'success'
                })
                emit('save')
              ...
              requestAddDataSource({
                ...
                .then(() => {
                  requestGenerateDataSource(getAppId())
                  useNotify({
                    title: t('designer.datasource.dataSourceAddSuccess'),
                    type: 'success'
                  })
                  emit('save')
                  dataSourceState.dataSourceColumn = {}
                ...
                .catch((error) => {
                  message({ message: `${t('designer.datasource.dataSourceSaveFailed')}：${error?.message || ''}`, status: 'error' })
                })
            }
          })
          close()
