<template>
  <div class="data-source-setting-remote">
    <div>
      ...
      <div class="tabBox">
        <div class="title">{{ t('designer.datasource.requestSettings') }}</div>
        <tiny-tabs v-model="state.activeNameTabs" tab-style="button-card">
          <tiny-tab-item class="json-tab" :title="t('designer.datasource.requestJsonParams')" name="jsonsTab">
            <data-source-remote-parameter v-model="state.remoteData.options.params"></data-source-remote-parameter>
          </tiny-tab-item>
          <tiny-tab-item :title="t('designer.datasource.requestProcessing')" name="responseTab">
    ...
    <tiny-button type="primary" @click.stop="sendRequest" class="send">{{ t('designer.datasource.viewRemoteFields') }}</tiny-button>
  </div>
</template>
  ...
  setup(props, { emit }) {
    ...
    const sendRequest = async () => {
      try {
        ...
        throw new Error(`${t('designer.datasource.formValidationRequired')}: ${error?.message || ''}`)
      }
      ...
      request
        ...
        .catch((error) => {
          useNotify({
            ...
            message: error.message || t('designer.datasource.requestFailedMessage')
          })
        })
    }
