...
// 数据源管理 -- 获取数据源列表
export const fetchDataSourceList = appId =>
...
export const requestDeleteDataSource = dataSourceId =>
    getMetaApi(META_SERVICE.Http).get(
        `/app-center/api/sources/delete/${dataSourceId}`
    );

// 数据源管理 -- 更新数据源
export const requestUpdateDataSource = (dataSourceId, params) =>
    getMetaApi(META_SERVICE.Http).post(
        `/app-center/api/sources/update/${dataSourceId}`,
        params
    );
...
export const fetchDataSourceDetail = dataSourceId =>
    getMetaApi(META_SERVICE.Http).get(
        `/app-center/api/sources/detail/${dataSourceId}`
    );

// 数据源管理 -- 获取数据源模板
export const fetchTemplates = platformId =>
    getMetaApi(META_SERVICE.Http).get(
        `/app-center/api/source_tpl?platform=${platformId}`
    );

// 数据源管理 -- 查询数据源模板详情
export const fetchTemplateDetail = templateId =>
    getMetaApi(META_SERVICE.Http).get(
        `/app-center/api/source_tpl?id=${templateId}`
    );

// 数据源管理 -- 设置全局的请求成功后的处理函数
export const requestGlobalDataHandler = (appId, params) =>
    getMetaApi(META_SERVICE.Http).post(
        `/app-center/api/apps/update/${appId}`,
        params
    );
