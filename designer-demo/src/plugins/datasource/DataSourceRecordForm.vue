        ...
        <template #content>
            <div class="record-form-content">
                <slot>
                    ...
                    >
                        <tiny-form-item
                            v-for="item in state.filterData"
                            :key="item.id"
                            :prop="item.name"
                        >
                            <template #label>
                                <span
                                    v-if="item.format?.required"
                                    class="dot-tip"
                                    >*</span>
                                {{ item.title }}
                            </template>
                ...
                <div v-if="showFooter" class="record-form-footer">
                    <tiny-button class="del" @click="$emit('delete')">
                        <icon-del />
                        <span>{{ t('designer.common.delete') }}</span>
                    </tiny-button>
                    <tiny-button @click="$emit('duplicate', recordFormData)">
                        <icon-copy />
                        <span>{{ t('designer.common.copy') }}</span>
                    </tiny-button>
                </div>
            </div>
        </template>
...
export const init = data => {
    const { dataSourceState } = useDataSource();
    data?.forEach(item => {
        recordFormData[item.name] =
            item.type === CONSTANTS.FIELD_TYPE_NUMBER ? 0 : '';
    });
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const state = reactive({
            ...
            rules: {},
            filterData: computed(() =>
                props.data.filter(item => item.field !== 'id')
            ),
            recordMapping: {}
        });
                ...
                value.forEach(item => {
                    ...
                    if (!isEmptyObject(format)) {
                        Object.keys(format).forEach(key => {
                            ...
                            if (
                                (key === CONSTANTS.MIN && format[key] !== 0) ||
                                (key === CONSTANTS.MAX && format[key] !== 0)
                            ) {
        ...
        watch(
            ...
            value => {
                ...
                dataSourceState.isRecordValidate = Object.keys(
                    state.recordMapping
                ).every(key => state.recordMapping[key] && value[key]);
            },
            { deep: true }
        );
