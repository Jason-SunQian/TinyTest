...
<template>
    ...
    >
        <template #header>
            <button-group>
                <tiny-button type="info" @click="saveGlobalDataHandle">{{
                    t('designer.common.save')
                }}</tiny-button>
                <svg-button name="close" @click="close" />
            </button-group>
        </template>
        <template #content>
            <tiny-collapse v-model="activeNames">
                <tiny-collapse-item
                    :title="t('designer.datasource.willFetchHandler')"
                    name="willFetch"
                >
                    <data-handler-editor v-model="state.willFetchValue" />
                </tiny-collapse-item>
                <tiny-collapse-item
                    :title="t('designer.datasource.dataHandler')"
                    name="dataHandler"
                >
                    <data-handler-editor v-model="state.dataHandlerValue" />
                </tiny-collapse-item>
                <tiny-collapse-item
                    :title="t('designer.datasource.errorHandler')"
                    name="errorHandler"
                >
                    <data-handler-editor v-model="state.errorHandlerValue" />
                </tiny-collapse-item>
            </tiny-collapse>
        </template>
    </plugin-setting>
    ...
    setup() {
        ...
        const saveGlobalDataHandle = () => {
            ...
            requestGlobalDataHandler(id, { data_source_global: handler }).then(
                data => {
                    if (data) {
                        useResource().appSchemaState.dataHandler = {
                            type: 'JSFunction',
                            value: state.dataHandlerValue
                        };
                        useResource().appSchemaState.willFetch = {
                            type: 'JSFunction',
                            value: state.willFetchValue
                        };
                        useResource().appSchemaState.errorHandler = {
                            type: 'JSFunction',
                            value: state.errorHandlerValue
                        };
                        confirm({
                            title: t('designer.datasource.tip'),
                            message: t(
                                'designer.datasource.globalHandlerSetSuccess'
                            )
                        });
                    }
                }
