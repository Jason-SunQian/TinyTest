<template>
    <div>
        <div v-if="state.columns.length" class="mapping-list">
            <div
                v-for="item in state.columns"
                :key="item.id"
                class="mapping-list-item"
            >
                <div class="item-body">
                    <span>{{ item.name }}</span>
                    <tiny-select
                        ...
                        :options="mappingType"
                        :placeholder="
                            t('designer.datasource.selectFieldInputType')
                        "
                        text-field="name"
                        value-field="type"
                    />
                    <tiny-input
                        v-model="item.title"
                        :placeholder="t('designer.datasource.enterLabelName')"
                    />
                    <div class="open">
                        <tiny-checkbox
                            v-model="item.status"
                        ...
                        />
                        <span class="tip">{{
                            t('designer.datasource.enableField')
                        }}</span>
                    </div>
                </div>
            </div>
        ...
        <div v-else class="mapping-empty-wrap">
            ...
            <p class="empty-text">{{ t('designer.common.noData') }}</p>
        </div>
    </div>
</template>
