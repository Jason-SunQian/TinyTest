<!-- eslint-disable vue/multi-word-component-names, vue/max-lines-per-block, vue/block-lang, vue/require-typed-object-prop -->
<template>
    <div
        :class="[
            'draggable-tree',
        ...
        if (node.parentId !== props.rootId) {
            ...
            for (let i = parentIndex + 1; i < index; i++) {
                result[i][node.level - 1] =
                    (result[i][node.level - 1] || 0) | lines.layer;
            }
        }
    }
