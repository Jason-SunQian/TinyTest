    ...
    >
        <tiny-form-item :label="t('designer.block.eventName')" prop="eventName">
            <tiny-input
                ...
                class="event-name"
                :placeholder="eventNameTip"
                @blur="changeEventName"
            >
                <template #suffix>
                    <tiny-popover
                    ...
                    >
                        ...
                        <div v-show="state.showPopover" class="property-list">
                            ...
                            <ul class="property-list-content">
                                <li
                                    ...
                                    :key="index"
                                    :class="{
                                        existed: eventNameList.has(
                                            `onUpdate:${item.property}`
                                        )
                                    }"
        ...
        <tiny-form-item :label="t('designer.common.description')">
            <tiny-input v-model="description" />
        </tiny-form-item>
        <div v-if="linked" class="linked-info">
            {{ t('designer.block.linkedToComponent') }}:
            {{ linked.componentName }} {{ t('designer.block.event') }}:
            {{ linked.event }}
        </div>
    </tiny-form>
</template>
...
export default {
    ...
    setup() {
        ...
        const eventNameList = computed(
            () => new Set(Object.keys(events.value))
        );
            ...
            eventName: [
                {
                    ...
                    validator: (
                        ...
                        if (isUpdateEvent.value) {
                            const matched = /^onUpdate:[a-zA-Z_$][\w$]*$/.test(
                                value
                            );
                            const propertyMatched = propertys.value.some(
                                item =>
                                    item.property ===
                                    value.replace('onUpdate:', '')
                            );
                            if (matched && propertyMatched) {
                                callback();
                            } else {
                                callback(
                                    new Error(
                                        t(
                                            'designer.block.eventNamePropertyError',
                                            {
                                                eventName: value,
