/**
 * Migrated from packages/plugins/block/src/js/blockSetting.tsx
 */
 ...
 * BUT WITHOUT ANY WARRANTY, INCLUDING BUT NOT LIMITED TO THE IMPLIED WARRANTY OF MERCHANTABILITY
...
import { t as i18nServiceT } from '../../../services/i18nService';
...
export const META_TYPES_OPTIONS = Object.entries(META_TYPES).map(
    ([key, value]) => ({
        label: key,
        value
    })
...
export const setEditProperty = property => {
    ...
    state.arrayConfig = (property?.properties?.[0]?.content || []).map(
        ({
            ...
            defaultValue,
            label,
            widget: { props = {}, component = 'InputConfigurator' } = {}
        }) => ({
            property: propertyName,
            type,
            defaultValue,
...
export const getEditBlockPropertyList = () =>
    state.block?.content?.schema?.properties?.[0]?.content;
...
export const delBlockEvent = eventName => {
    const block = getEditBlock();
    if (block?.content?.schema?.events) {
        delete block.content.schema.events[eventName];
    }
};

export const renameBlockEventName = (newName, oldName) => {
    const block = getEditBlock();
    if (block?.content?.schema?.events && oldName) {
        const { events } = block.content.schema;
        if (events[oldName]) {
            events[newName] = events[oldName];
...
export const updateBlockList = () => {
    ...
    fetchBlockList({ appId, ...params }).then(data => {
        const blockListDescByUpdateAt = data.sort(
    ...
    if (blockId) {
        requestDeleteBlock(blockId)
            .then(() => {
                ...
                message({
                    message: t('designer.block.deleteBlockSuccess'),
                    status: 'success'
                });
                updateBlockList();
        ...
        if (STRING_SLOT.includes(componentName)) {
            ...
            const slotParams =
                typeof props.params === 'string'
                    ? props.params.split(',')
                    : props.params?.map(item => item.name);
    ...
    if (block && validBlockSlotsName(block)) {
        requestDeployBlock(params)
            .then(() => {
                ...
                useNotify({
                    message: t('designer.block.publishBlockSuccess'),
                    type: 'success'
                });
                updateBlockList();
                useBlock().isRefresh.value = true;
...
const getCategories = () => {
    const appId = getAppId();
    const fetchData = useBlock().shouldReplaceCategoryWithGroup()
        ? fetchGroups
        : fetchCategories;
...
const createBlock = (block = {}) => {
    ...
    return requestCreateBlock(params)
        .then(data => {
            ...
            useNotify({
                message: t('designer.block.createBlockSuccess'),
                type: 'success'
            });
            // 本地生成区块服务
            if (isVsCodeEnv) {
...
const updateBlock = (block = {}) => {
    ...
    return requestUpdateBlock(
        ...
        .then(data => {
            ...
            // 弹出保存区块成功
            useNotify({
                message: t('designer.block.saveBlockSuccess'),
                type: 'success'
            });
            // 本地生成区块服务
            if (isVsCodeEnv) {
 ...
 * @returns
 */
const generateBlockDeps = (
    children,
    deps = { scripts: [], styles: new Set() }
) => {
    if (!Array.isArray(children)) {
        return;
    }
...
export const delCategory = async id => {
    const deleteFn = useBlock().shouldReplaceCategoryWithGroup()
        ? deleteGroup
        : deleteCategory;
        ...
        if (property === 'component') {
            props.options =
                dataType === META_TYPES.array
                    ? [
                          {
                              label: META_COMPONENTS[META_TYPES.array],
                              value: META_COMPONENTS[META_TYPES.array]
                          }
                      ]
                    : (META_COMPONENT_LIST[dataType] || []).map(item => ({
                          label: item,
                          value: item
                      }));
        }
