...
const typeList = Object.keys(META_TYPES).map(key => ({
    value: key,
    label: key
}));
...
const showArrayItemConfig = ref(false);
...
const type = computed({
    ...
    set(value) {
        ...
        property.widget = {
            component:
                META_COMPONENTS[value] || META_COMPONENTS[META_TYPES.string],
            props: {}
        };
    }
});
...
const widgetComponent = computed({
    ...
    set(value) {
        ...
        showArrayItemConfig.value =
            type.value === META_TYPES.array &&
            value === META_COMPONENTS_ENUM.ArrayItemConfigurator;

        initSubProperties();
    }
});

const showPropertyConfigItem = computed(() => getConfigItemVisible());
...
const getDefaultValue = data => {
    ...
    if (
        ...
        propertyType === META_TYPES.function
    ) {
        return data;
    }

    if (propertyType === META_TYPES.boolean) {
        return Boolean(data);
    }

    if (propertyType === META_TYPES.number) {
        return Number(data);
    }
    ...
    if (
        propertyType === META_TYPES.object &&
    ...
    if (propertyType === META_TYPES.array && Array.isArray(newValue)) {
        return newValue;
    }
...
// 设置区块属性访问器
const saveAccessor = (accessorType, data) => {
    const property = getEditProperty();
    property.accessor = property.accessor || {};
    property.accessor[accessorType] = {
        type: 'JSFunction',
        value: data.content
    };
};

const handleConfigItemChange = (property, data) => {
    if (property === 'defaultValue') {
        ...
        Object.prototype.hasOwnProperty.call(currentEditData.value, property)
    ) {
...
const handleSaveWidgetProps = data => {
    const property = getEditProperty();
    if (!property.widget) {
        property.widget = {};
    }
    ...
    try {
        ...
        useNotify({
            type: 'error',
            message: `属性面板组件属性值设置有误: ${JSON.stringify(
                error
            )}，请检查是否为JSON类型`
        });
    }
...
const handleChangeWidgetComponent = value => {
...
watch(
    ...
    () => {
        ...
        widgetComponentList.value = (
            META_COMPONENT_LIST[(type.value || '').toLowerCase()] || []
        ).map(item => ({
            label: item,
            value: item
        }));
