...
const i18nDataSchema = z.object({
    entries: z
        .record(z.any())
        .describe('I18n entries object containing key-value pairs'),
    ...
    callback: (args: z.infer<typeof inputSchema>) => {
        ...
        try {
            ...
            if (key) {
                if (!langs[key]) {
                    ...
                    return createErrorResponse(
                        'I18n key not found',
                        `Key "${key}" does not exist in the i18n dictionary`
                    );
                }
                ...
                return createSuccessResponse(
                    `I18n entry for key "${key}" retrieved successfully`,
                    singleEntryData
                );
            }
            ...
            const res = createSuccessResponse(
                `Retrieved ${entryCount} i18n entries`,
                {
                    entries: langs,
                    count: entryCount
                }
            ...
            return createErrorResponse(
                'Failed to retrieve i18n entries',
                errorMessage
            );
        }
    }
