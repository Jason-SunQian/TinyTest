  ...
  >
    <template #content>
      <div class="language-search-box">
        ...
        <tiny-input v-model="searchKey" class="plugin-i18n-search" :placeholder="t('designer.i18n.searchPlaceholder')" type="text" clearable>
          <template #prefix>
            <span class="icon">
      ...
      <div class="btn-box">
        <tiny-button @click="openEditor($event, {})">
          <svg-icon name="add" class="btn-icon"></svg-icon>{{ t('designer.i18n.addEntry') }}
        </tiny-button>
        <tiny-button class="middle-btn" @click="batchDelete" :disabled="!selectedRowLength"
          ><svg-icon class="btn-icon" name="delete"></svg-icon>{{ t('designer.i18n.delete') }}</tiny-button
        >
        <tiny-file-upload
          ref="upload"
          size="small"
        ...
        >
          <template #trigger>
            <tiny-button><svg-icon class="btn-icon" name="upload"></svg-icon>{{ t('designer.i18n.batchUpload') }}</tiny-button>
          </template>
        </tiny-file-upload>
        <a class="download-btn" @click="downloadFile"> {{ t('designer.i18n.downloadTemplate') }} </a>
        <p v-show="isLoading && notEmpty">
          <span id="boxeight" class="i18n-loading"></span><span>{{ t('designer.i18n.importing') }}</span>
        </p>
      </div>
      <div class="language-plugin-table lowcode-scrollbar">
        ...
        >
          ...
          <tiny-grid-column
            ...
            :editor="{ component: 'input', autoselect: true }"
          ></tiny-grid-column>
          <tiny-grid-column width="90" field="operation" :title="t('designer.i18n.operation')">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const validRules = computed(() => ({
      key: [{ required: true, message: t('designer.common.required') }, { validator: validateKey }]
    }))
    ...
    const batchDelete = () => {
      ...
      confirm({
        ...
        message: t('designer.i18n.confirmDelete', { count: i18nData.length }),
        exec: () => {
    ...
    const handleChange = (data) => {
      ...
      const loadintText = notEmpty.value ? '' : t('designer.i18n.importing')
