...
<template>
    <div
        :class="[
            'draggable-tree',
            'border-transparent',
            { 'hover-border-color': hoveringNodeId === rootId }
        ]"
    ...
    >
        ...
        >
            <div class="content" @click="handleClickRow($event, node)">
                <layer-lines
                    :line-data="layerLineComputed[rowIndex]"
                    :level="node.level"
                />
                <div class="prefix-icon" @click="handleSwitchCollapse(node)">
                    <svg-icon
                        ...
                        :name="
                            collapseMap[node.id] ? 'page-collection' : 'page'
                        "
                    />
                    <svg-icon
                        v-else
                        :name="collapseMap[node.id] ? 'folder' : 'folder-wold'"
                    />
                </div>
                <label>{{ node.label }}</label>
            </div>
            <slot name="row-suffix" :node="node" />
        </div>
    </div>
</template>
...
const emit = defineEmits<{
    clickRow: [node: TreeNode];
    moveNode: [dragged: TreeNode, target: Pick<TreeNode, 'id'>];
}>();
        ...
        for (const child of children) {
            result.push(
                ...flattenTreeData(
                    ...
                    currentNode.id,
                    level + 1,
                    collapsed || collapseMap.value[currentNode.id]
                )
            );
        }
    }
...
const getAncestorIds = (nodeId: string | number): Array<string | number> => {
...
watch(nodes, nodesValue => {
    filteredNodes.value = nodesValue.filter(node =>
...
const nodeLine = 0b01;
// │
const layerLineValue = 0b10;
// ├
const layerNodeLine = 0b11;
const lines = {
    node: nodeLine,
    layer: layerLineValue,
    layerNode: layerNodeLine
};

const layerLineComputed = computed(() => {
    const result: Record<number, Record<number, number>> = {};

    const filteredNodesValue = filteredNodesWithAncestors.value;

    for (const [index, node] of filteredNodesValue.entries()) {
        result[index] = result[index] || {};
        result[index][node.level - 1] = lines.node;

        if (node.parentId !== props.rootId) {
            const parentIndex = filteredNodesValue.findIndex(
                item => item.id === node.parentId
            );
            for (let i = parentIndex + 1; i < index; i++) {
                result[i][node.level - 1] =
                    (result[i][node.level - 1] || 0) | layerLineValue;
            }
        }
    }
...
const handleDragOver = (event: DragEvent, node: TreeNode) => {
    ...
    const isDescendant = getAncestorIds(node.id).includes(
        ...
        * {
        ...
        &:hover {
            background-color: var(
                --te-page-manage-draggable-row-bg-color-hover
            );
        }
