<!-- eslint-disable vue/no-template-shadow, vue/max-lines-per-block -->
<template>
    <div class="app-manage-search">
        <tiny-search
            v-model="state.pageSearchValue"
            clearable
            :placeholder="t('designer.page.search')"
        >
            <template #prefix>
                <tiny-icon-search />
            </template>
    ...
    <tiny-collapse
        v-model="state.collapseValue"
        class="page-manage-collapse lowcode-scrollbar"
    >
        <tiny-collapse-item
            v-for="(groupItem, index) in pageSettingState.pages"
            :key="index"
            :name="groupItem.groupId"
        >
            <template #title>
            ...
            <div class="app-manage-tree">
                ...
                >
                    <template #row-suffix="{ node }">
                        <div :class="['actions']">
                            <svg-button
                                v-if="isPageLocked(node.rawData)"
                                name="locked"
                                :hover-bg-color="false"
                            />
                            <svg-button
                                v-if="node.rawData.isHome"
                                name="home"
                                :hover-bg-color="false"
                            />
                            <tiny-popover
                            ...
                            >
                                <div class="operation-list">
                                    <div
                                        ...
                                        :class="
                                            [
                                                operation.type === 'divider'
                                                    ? 'divider'
                                                    : 'item'
                                            ].concat(operation.class || [])
                                        "
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const options = getOptions(meta.id);
        const { customPageOperations = [] } = options;

        const baseInfo = getMetaApi(META_SERVICE.GlobalService).getBaseInfo();
        const { pageId } = baseInfo;
        ...
        pageSettingState.updateTreeData = async () => {
            const appId = getAppId();
            const pageList = await refreshPageList(appId);
            return pageList;
        };
        ...
        const isPageLocked = pageData => {
            return (
                getCanvasStatus(pageData.occupier).state === PAGE_STATUS.Lock
            );
        };
        ...
        const rowOperations = [
            {
                type: 'settings',
                label: '设置',
                action: handleClickPageSettings
            },
            { type: 'divider' },
        ...
        const getRowOperations = (groupId, node) => {
            ...
            if (!node.rawData.isPage || node.rawData.group === 'public') {
                return rowOperations.filter(
                    item => item.type !== 'settingHome'
                );
            }
            // TODO 复制逻辑暂时屏蔽
        ...
        .item {
            ...
            &:hover {
                background-color: var(
                    --te-page-manage-operation-list-item-bg-color-hover
                );
            }
        }
        .divider {
            ...
            &::after {
                ...
                height: 1px;
                background-color: var(
                    --te-page-manage-operation-list-item-border-color-divider
                );
            }
        }
        .danger {
