<!-- eslint-disable vue/no-bare-strings-in-template, vue/html-self-closing -->
<template>
    <div :class="['block-detail-panel', { 'no-backup': !detail?.backup }]">
        ...
        <div class="block-detail-content">
            <div class="block-detail-item">
                ...
                <div class="block-item-history">
                    {{ detail?.current_history?.updated_at }}
                </div>
            </div>
            <div
            ...
            <div
                v-if="
                    !isDefaultGroupId(selectedGroup.groupId) &&
                    !isSettingPanel &&
                    !isShortcutPanel
                "
                class="block-delete"
...
<!-- eslint-disable vue/block-lang, vue/component-api-style, vue/require-explicit-emits -->
<script lang="tsx">
...
export default {
    ...
    emits: ['close'],
    setup(props, { emit }) {
        const { isRefresh, isDefaultGroupId, selectedBlock, selectedGroup } =
            useBlock();
            ...
            const exec = () => {
                fetchGroupBlocksById({ groupId })
                    ...
                    .catch(error => {
                        emit('close');
                        message({
                            message: `删除区块失败: ${error.message || error}`,
                            status: 'error'
                        });
                    });
            };
    ...
    .block-detail-title {
        ...
        .block-detail-icon {
            ...
            &:hover {
                ...
                background: var(
                    --te-materials-block-detail-icon-bg-color-hover
                );
            }
        }
    }
    ...
    .block-detail-content {
        &:not(:last-child) {
            border-bottom: 1px solid
                var(--te-materials-block-detail-border-color);
        }
