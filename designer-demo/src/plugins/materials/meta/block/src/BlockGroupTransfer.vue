...
<template>
    <div class="block-add-transfer">
        ...
        <div class="block-add-transfer-body">
            <block-list
                v-bind="$attrs"
                :block-list="state.blockList"
                :trigger-check="true"
                :show-block-shot="false"
                :show-setting-icon="false"
                :show-checkbox="true"
                :checked="selectedBlockArray"
                :grid-columns="6"
                @check="checkBlock"
                @check-all="checkAll"
                @cancel-check-all="cancelCheckAll"
            />
        </div>
    </div>
</template>
            ...
            if (!block.latestVersion) {
                fetchBlockById(block.id).then(blockDetail => {
                    ...
                    if (historyLength <= 0) {
                        ...
                        message({
                            message: `${blockDetail.label}区块缺少历史记录，请重新发布区块`,
                            status: 'error'
                        });
                    } else {
                        block.latestVersion =
                            blockDetail?.histories[historyLength - 1]?.version;
                        check(block);
                    }
                });
            } else {
