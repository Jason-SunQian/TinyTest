  ...
  >
    <template #content>
      <div class="block-add-content">
        <div class="block-add-content-title">{{ t('designer.materials.blockList') }}</div>
        <block-group-filters
        ...
        <block-group-transfer :blockList="state.blockList">
          <template #search>
            <tiny-search
              ...
              :placeholder="t('designer.materials.searchByNameOrId')"
  ...
  setup() {
    ...
    const state = reactive({
      ...
      filters: [
        {
          id: 'publicType',
          get name() { return t('designer.materials.byPublicScope') },
          children: []
        },
        {
          id: 'author',
          get name() { return t('designer.materials.byAuthor') },
          children: [],
          usingSelect: true
        },
        {
          id: 'tag',
          get name() { return t('designer.materials.byTag') },
          children: [],
          usingSelect: true
        }
      ],
    ...
    const addBlocks = () => {
      ...
      fetchGroupBlocksById({ groupId })
        ...
        .catch((error) => {
          message({
            message: `${t('designer.materials.addBlockFailed')}: ${error.message || error}`,
            status: 'error'
          })
        })
        .finally(() => {
    ...
    const searchBlocks = (filters) => {
      ...
      fetchAvailableBlocks(params)
        ...
        .catch((error) => {
          message({
            message: `${t('designer.materials.blockSearchFailed')}: ${error.message || error}`,
            status: 'error'
          })
        })
    }
    ...
    const fetchBlocks = (groupId) => {
      fetchAvailableBlocks({ groupId })
        ...
        .catch((error) => {
          message({
            message: `${t('designer.materials.fetchAvailableBlocksFailed')}: ${error.message || error}`,
            status: 'error'
          })
        })
    }
    ...
    const getFilters = () => {
      Promise.allSettled([fetchTenants(), fetchUsers(), fetchTags()]).then((results) => {
        state.filters[0].children = [
          {
            get name() { return t('designer.materials.publicToAll') },
            id: '1'
          },
          {
            get name() { return t('designer.materials.publicToCurrent') },
            id: '2'
          }
        ]
