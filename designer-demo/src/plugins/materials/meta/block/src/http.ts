    ...
    for (const item in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, item)) {
            ...
            if (itemValue instanceof Array) {
                itemValue.forEach(i => {
                    params.push(`${itemKey}=${i}`);
                });
            } else if (itemValue) {
                params.push(`${itemKey}=${itemValue}`);
            }
        }
    }
...
export const fetchGroups = appId =>
    getMetaApi(META_SERVICE.Http).get(
        `/material-center/api/block-groups?app=${appId}`
    );
...
export const fetchGroupBlocksById = async ({ groupId }) => {
    ...
    data = data
        .filter(
            block => block.framework === getMergeMeta('engine.config')?.dslMode
        )
...
// 更新区块版本
export const requestGroupBlockVersion = async ({
    groupId,
    blockId,
    blockVersion
}) => {
...
export const fetchTags = () =>
    getMetaApi(META_SERVICE.Http).get(`/material-center/api/block/tags`);

// 获取区块所有作者
export const fetchUsers = () =>
    getMetaApi(META_SERVICE.Http).get(`/material-center/api/block/users`);

// 获取区块所有tag
export const fetchTenants = () =>
    getMetaApi(META_SERVICE.Http).get(`/material-center/api/block/tenants`);
