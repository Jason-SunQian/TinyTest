<!-- eslint-disable vue/html-self-closing, vue/v-on-event-hyphenation, vue/max-lines-per-block, vue/block-lang, vue/component-api-style -->
<template>
    <plugin-block-list
        ref="blockRef"
        :data="blockList"
        ...
        :default-icon-tip="t('designer.materials.viewBlockDetail')"
        @click="blockClick"
...
import { computed, inject, reactive, ref, h } from 'vue';
import {
    useBlock,
    useMaterial,
    useModal,
    ...
    */
    ...
    setup(props, { emit }) {
        ...
        const state = reactive({
            ...
            detail: null,
            showShot: computed(
                () => props.showBlockShot && !panelState.isShortcutPanel
            )
        });
        ...
        const openDetail = ({ item, index }) => {
            const nodeItem = blockRef.value.$el.querySelector(
                `.block-item:nth-child(${index + 1})`
            );
            ...
            const offset =
                index % 2 ? itemOffsetWidth * 0.9 : -itemOffsetWidth * 0.1;
            const offsetWidth =
                panelState.isBlockGroupPanel && panelState.isBlockList
                    ? -(itemOffsetWidth / 8)
                    : itemOffsetWidth / 4;
            const location =
                index % 2
                    ? `calc(50% + ${itemOffsetWidth / 2}px)`
                    : `calc(50% - ${itemOffsetWidth / 2}px)`;
            const itemOffset =
                !panelState.isShortcutPanel && !panelState.isBlockGroupPanel
                    ? offset
                    : offsetWidth;
            const itemLocation =
                !panelState.isShortcutPanel && !panelState.isBlockGroupPanel
        ...
        const deleteBlock = ({
            ...
            const exec = () => {
                fetchGroupBlocksById({ groupId })
                    ...
                    .catch(error => {
                        message({
                            message: `移除区块失败: ${error.message || error}`,
                            status: 'error'
                        });
                    });
            };
