<template>
  <div class="toolbar-lang">
    <toolbar-base :content="currentLanguageName" :icon="options.icon || 'cn-en'" :options="options" @click-api="toggle">
      <template #default>
        <tiny-popover :visible-arrow="false" width="220" trigger="click">
          <template #reference>
            <span class="lang-btn">
              <span class="lang-flag">{{ currentLanguageFlag }}</span>
              <span class="lang-name">{{ currentLanguageName }}</span>
              <svg-icon name="down-arrow" class="lang-arrow"></svg-icon>
            </span>
          </template>
          <div class="lang-panel">
            <div class="lang-list">
              ...
              >
                ...
                <svg-icon v-if="lang.code === currentLanguage" name="check" class="lang-check"></svg-icon>
              </div>
            </div>
          </div>
        </tiny-popover>
...
import { useDesignerI18n, switchLanguage, getCurrentLanguage, getSupportedLanguages, type LanguageConfig } from '../../services/i18nService'
...
export default {
  ...
  setup() {
    ...
    const supportedLanguages = ref<LanguageConfig[]>(getSupportedLanguages())
    ...
    const currentLanguageName = computed(() => currentLanguageConfig.value?.name || '中文')
    const currentLanguageFlag = computed(() => currentLanguageConfig.value?.flag || '🇨🇳')
