      ...
      <div class="top-panel-breadcrumb">
        <div
          :class="[
            ...
            { 'top-panel-breadcrumb-title-block': currentType === 'block' }
          ]"
        >
...
import { useLayout, useBlock, useBreadcrumb } from '@opentiny/tiny-engine-meta-register'
...
export default {
  ...
  setup() {
    ...
    const breadcrumbData = computed(() => {
      const prefix = currentType.value === 'page' ? CONSTANTS.value.PAGETEXT : CONSTANTS.value.BLOCKTEXT
      const data = rawData.value && rawData.value.length > 0 ? rawData.value : []
    ...
    const open = () => {
      if (!plugins) return
      plugins.render = currentType.value === 'page' ? PLUGINS_ID.PAGEID : PLUGINS_ID.BLOCKID
    }
