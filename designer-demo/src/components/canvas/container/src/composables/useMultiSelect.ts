...
export const useMultiSelect = () => {
    ...
    const toggleMultiSelection = (
        ...
        // 多选
        if (isMultiple) {
            const isExistNode = multiSelectedStates.value.some(
                state => state.id === selectState.id
            );
            // 如果多选列表已经存在选中的state且鼠标抬起，则将选中的state移出多选列表
            if (isExistNode && !isMouseDown.value) {
                multiSelectedStates.value = multiSelectedStates.value.filter(
                    state => state.id !== selectState.id
                );
            } else {
                multiSelectedStates.value =
                    multiSelectedStates.value.concat(selectState);
            }
    ...
    const areSiblingNodes = (): boolean => {
        ...
        const nodesWithParent = multiSelectedStates.value.map(
            node => canvas.getNodeWithParentById(node.id) || {}
        );
        ...
        if (nodesWithParent.some(node => node.parent.id !== parentId))
            return false;

        // 获取并检查索引
        const selectedIds = multiSelectedStates.value.map(state => state.id);
        const nodeIndices = getSelectedNodeIndices(
            firstParent.children,
            selectedIds
        );

        // 检查索引是否连续
        return nodeIndices.every(
            (value, index) => value === nodeIndices[0] + index
        );
    };
    ...
    const getComponentBaseStyleClassName = () => {
        ...
        return materialsOptions.useBaseStyle &&
    ...
    const groupAddParent = (
        ...
        const { parent } =
            useCanvas().getNodeWithParentById(firstState.id) || {};
        ...
        if (componentName === 'TinyPopover') {
            wrapSchema = createTinyPopoverSchema(selectedNodes, props);
        }
    ...
    const batchAddParent = (
        ...
        multiSelectedStates.value.forEach(({ schema, parent }) => {
            ...
            // 创建包装组件的模板，并确保有唯一ID
            const wrapSchema = createWrapperSchema(
                componentName,
                schema,
                props
            );
