...
export const useMultiDrag = () => {
    const multiDragState = reactive<MultiDragState>({
        ...initialMultiDragState
    });
    const { multiSelectedStates } = useMultiSelect();
    const multiStateLength = computed<number>(
        () => (multiSelectedStates.value as SelectState[]).length
    );
    ...
    const startMultiDrag = (
        ...
        if (
            !clickedNodeId ||
            !(multiSelectedStates.value as SelectState[]).some(
        ...
        multiDragState.nodes = toRaw(
            multiSelectedStates.value as SelectState[]
    ...
    const checkAllowInsert = (
        ...
        const { parent: targetParent } =
            useCanvas().getNodeWithParentById(targetId) || {};
        ...
        for (const node of nodes) {
            ...
            if (position === POSITION.IN) {
                ...
                // 放置到节点前后需要检查父节点的配置
                const parentConfigure = targetParent
                    ? getConfigure(targetParent.componentName)
                    : { isContainer: true };
    ...
    const initDragState = (currentMousePos: Position): boolean => {
        if (!multiDragState.dragStarted) {
            const distance = calculateDistance(
                multiDragState.initialMousePos,
                currentMousePos
            );
    ...
    const handleBodyPlacement = (
        ...
        for (const childSchema of bodyChildren) {
            ...
            if (distance < closestDistance) {
                ...
                // 判断放置位置：在节点上方还是下方
                position =
                    clientY < childMiddle ? POSITION.TOP : POSITION.BOTTOM;
            }
        }
        ...
        if (closestNode) {
            ...
            // 检查是否允许放置
            const isForbidden = !checkAllowInsert(
                configure,
                multiDragState.nodes,
                nodeId!,
                position
            );
    ...
    const handleSelfNodeDrag = (
        ...
        const targetIndex = children.findIndex(
            (child: NodeSchema) => child.id === targetId
        );
        ...
        if (
            ...
            if (
                nextSibling &&
                !multiDragState.nodes.some(node => node.id === nextSibling.id)
            ) {
                // 使用下一个兄弟节点作为目标
                ...
                if (nextElement) {
                    ...
                    const nextComponentName =
                        nextElement.getAttribute(NODE_TAG);
    ...
    // 处理容器内放置
    const handleContainerPlacement = (
        targetId: string,
        rect: DOMRect,
        configure: { isContainer?: boolean } | null,
        isForbidden: boolean
        ...
        const { node } =
            targetId === 'body'
                ? { node: getSchema() }
                : getNodeWithParentById(targetId) || {};
    ...
    const moveMultiDrag = (event: MouseEvent): boolean => {
        if (!multiDragState.keydown || multiStateLength.value <= 1)
            return false;
        ...
        // 检查是否是拖拽自身节点
        const isDraggingSelf = multiDragState.nodes.some(
    ...
    const getTargetNodeInfo = (targetId: string) => {
        const { getNodeWithParentById, getSchema } = useCanvas();
        const { node: targetNode, parent: targetParent } =
            getNodeWithParentById(targetId) || {};
    ...
    // 收集拖拽操作
    const collectDragOperations = (
        targetInfo: {
            finalTargetNode: NodeSchema | null;
            finalTargetParent: NodeSchema | null;
            isBodyTarget: boolean;
            targetNode: NodeSchema | null;
            targetParent: NodeSchema | null;
        },
        position: PositionType | null
    ): InsertOperation[] => {
        ...
        multiDragState.nodes.forEach(node => {
            ...
            const { node: sourceNode, parent: sourceParent } =
                useCanvas().getNodeWithParentById(sourceId) || {};
            ...
            if (
                position !== POSITION.IN &&
                finalTargetParent &&
                movingNodeIds.includes(finalTargetParent.id)
            ) {
                return;
            }
    ...
    const insertNodeToTarget = (
        ...
        if (isBodyTarget) {
            // 需要构建正确的目标节点数据
            const { getNodeWithParentById } = useCanvas();
            const { node: targetChildNode, parent: targetChildParent } =
                getNodeWithParentById(targetId) || {};
            ...
            insertNode(
                { node: useCanvas().getSchema(), data: op.targetNodeData.data },
                POSITION.IN,
                false
            );
        } else {
    ...
    const updateMultiSelectionAfterDrag = (operations: InsertOperation[]) => {
        ...
        setTimeout(() => {
            ...
            newNodeIds.forEach(nodeId => {
                ...
                if (element) {
                    const { node } =
                        useCanvas().getNodeWithParentById(nodeId) || {};
                    if (!node) return;
    ...
    // 执行拖拽操作
    const executeDragOperations = (
        operations: InsertOperation[],
        targetInfo: { isBodyTarget: boolean }
    ) => {
        ...
        const sortedOperations = sortOperationsByPosition(
            operations,
            positions,
            position
        );
    ...
    const endMultiDrag = (): boolean => {
        ...
        // 收集拖拽操作
        const operations = collectDragOperations(
            targetInfo,
    ...
    const getMultiDragPositionText: ComputedRef<string> = computed(() => {
        ...
        if (id && id !== 'body') {
            ...
            if (targetElement) {
                targetComponentName =
                    targetElement.getAttribute(NODE_TAG) || '';
            }
        } else if (id === 'body') {
