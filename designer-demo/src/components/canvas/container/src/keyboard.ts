...
const handleClipboardCut = event => {
    const selectedNodes = multiSelectedStates.value.map(({ schema }) =>
        copyObject(schema)
    );
...
const handleClipboardPaste = event => {
    ...
    const [lastSelected] = multiSelectedStates.value.slice(-1);
    ...
    nodeList.forEach(node => {
        if (
            ...
            allowInsert(getConfigure(schema.componentName), node)
        ) {
...
const handleCopyEvent = event => {
    ...
    // 验证所有选中的节点是否有效（不为空）
    const isValidNodes = selectedNodes.every(
