<!-- eslint-disable vue/no-root-v-if, vue/block-lang, vue/component-api-style, vue/html-self-closing, vue/require-typed-object-prop -->
<template>
    <div
        ...
        :title="
            state.targetPageId
                ? t('designer.canvas.routeJumpEdit')
                : t('designer.canvas.routeJumpNotBound')
        "
    ...
    setup(props) {
        ...
        watch(
            ...
            ([hoverState, inactiveHoverState]) => {
                ...
                state.top = `${top}px`;
                state.targetPageId =
                    element.getAttribute('data-router-target-page-id') || null;
            },
            { deep: true }
        );
