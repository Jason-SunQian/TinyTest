...
<!-- eslint-disable vue/no-bare-strings-in-template, vue/max-lines-per-block -->
<template>
    <div>
        <!-- 多选拖拽状态指示器 -->
        <div v-if="isMultiDragging" class="multi-drag-indicator">
            <span>{{
                t('designer.canvas.draggingComponents', multiStateLength)
            }}</span>
            <span v-if="getMultiDragPositionText" class="multi-drag-position">{{
                getMultiDragPositionText
            }}</span>
        </div>
        <!-- 多选拖拽可放置位置指示器 -->
        <div
            v-if="isMultiDragging && lineState.height && lineState.width"
            class="multi-drop-indicator"
        >
            <div
                :class="[
                    'multi-drop-line',
            ...
            >
                <div
                    v-if="lineState.position === 'in'"
                    class="multi-drop-container-hint"
                >
                    <span>{{
                        t(
                            'designer.canvas.dropComponentsToContainer',
                            multiStateLength
                        )
                    }}</span>
                </div>
        ...
        <!-- 拖拽时的节点预览 -->
        <div
            v-if="isMultiDragging && multiDragState.mouse"
            class="multi-drag-preview-container"
        >
            <div
            ...
            >
                ...
                <div class="preview-component-info">
                    <div class="preview-component-name">
                        {{ node.componentName }}
                    </div>
                    <div
                        v-if="node.props && node.props.id"
                        class="preview-component-id"
                    >
                        {{ `ID: ${node.props.id}` }}
                    </div>
                </div>
...
.multi-drag-indicator {
    ...
    background-color: var(
        --te-canvas-container-bg-color-checked,
        rgba(24, 144, 255, 0.9)
    );
...
.multi-drop-indicator {
    ...
    .multi-drop-line {
        ...
        &.in {
            ...
            border: 2px dashed
                var(--te-canvas-container-border-color-checked, #1890ff);
            display: flex;
        ...
        &.forbidden:not(.in) {
            background: var(
                --te-canvas-container-hover-line-forbid-bg-color,
                #ff4d4f
            );
        }
        &.forbidden.in {
            ...
            border: 2px dashed
                var(--te-canvas-container-hover-line-forbid-bg-color, #ff4d4f);
        }
