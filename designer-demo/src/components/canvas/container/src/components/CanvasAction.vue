...
<!-- eslint-disable vue/html-self-closing, vue/padding-line-between-blocks, vue/block-lang, import/order, @typescript-eslint/naming-convention, new-cap, vue/require-default-prop, vue/component-api-style, @typescript-eslint/prefer-destructuring, @typescript-eslint/explicit-member-accessibility, @typescript-eslint/no-misused-promises -->
<template>
    <div
        v-show="selectState.height && selectState.width"
        :class="[
            'canvas-rect select',
            { 'multi-select': multiStateLength > 1 },
            { dragging: isMultiDragging }
        ]"
        :style="{
    ...
    >
        ...
        >
            ...
            >
                ...
                <template #reference>
                    <icon-setting
                        class="icon-setting svg-currentcolor"
        ...
        <div
            v-if="showAction"
            ref="optionRef"
            class="corner-mark-right"
            :style="fixStyle"
        >
            <template v-if="!isModal">
                <div :title="selectParentTitle">
                    <icon-chevron-left
                        v-if="showToParent"
                        class="svg-currentcolor"
                        @click.stop="selectParent"
                    ></icon-chevron-left>
                </div>
                <div :title="moveUpTitle">
                    <icon-arrow-up
                        class="svg-currentcolor"
                        @click.stop="moveUp"
                    ></icon-arrow-up>
                </div>
                <div :title="moveDownTitle">
                    <icon-arrow-down
                        class="svg-currentcolor"
                        @click.stop="moveDown"
                    ></icon-arrow-down>
    ...
    >
        ...
        <div
            v-show="hoverState.configure?.isContainer"
            class="corner-mark-bottom-right"
        >
            {{ dragToContainerText }}
        </div>
    </div>
    <div
        v-show="inactiveHoverState.height && inactiveHoverState.width"
    ...
    <div v-show="lineState.height && lineState.width" class="canvas-rect line">
        <div
            :class="[
                'hover-line',
                lineState.position,
                { forbidden: lineState.forbidden }
            ]"
        >
            <div
                v-if="lineState.position === 'in' && hoverState.configure"
                class="choose-slots"
            ></div>
        </div>
    </div>
...
export default {
    ...
    setup(props) {
        ...
        const quickSettingsTitle = computed(() =>
            t('designer.canvas.quickSettings')
        );
        const selectParentTitle = computed(() =>
            t('designer.canvas.selectParent')
        );
        const moveUpTitle = computed(() => t('designer.canvas.moveUp'));
        const moveDownTitle = computed(() => t('designer.canvas.moveDown'));
        const copyTitle = computed(() => t('designer.canvas.copy'));
        const hideTitle = computed(() => t('designer.canvas.hide'));
        const deleteTitle = computed(() => t('designer.canvas.delete'));
        const dragToContainerText = computed(() =>
            t('designer.canvas.dragToContainer')
        );
        ...
        const selectParent = () => {
            ...
            if (
                parent?.componentName === 'Template' &&
                !querySelectById(parentId)
            ) {
                const grandParent =
                    useCanvas().getNodeWithParentById(parentId)?.parent;
        ...
        const showToParent = computed(
            () => getCurrent().parent !== useCanvas().getSchema()
        );

        const isModal = computed(() => {
            const config = useMaterial().getMaterial(
                props.selectState.componentName
            );
            ...
            styleObj2Str = styleObj => {
                return Object.entries(styleObj)
                    .map(([key, value]) => {
                        ...
                        const val = positions.isHorizontal(key)
                            ? num - SELECTION_BORDER_WIDTH
                            : num;
                        return `${key}:${val}px`;
                    })
                    .join(';');
        ...
        const getStyleValues = (
            ...
            const isOptionAtBottom =
                canvasHeight - top - height >= OPTION_BAR_HEIGHT;
            ...
            if (width < fullRectWidth) {
                ...
                const isLabelAlignRight =
                    labelWidth > width &&
                    left + labelWidth + scrollBarWidth > canvasWidth;
                if (isLabelAlignRight) {
                    labelAlign.align(positions.RIGHT);
                }

                // 如果option宽度大于选中框宽度，并且option左侧已经超出画布，则option对齐左侧
                const isOptionAlignLeft =
                    optionWidth > width && left + width - optionWidth < 0;
                ...
                if (left < 0) {
                    labelAlign.align(
                        positions.LEFT,
                        Math.min(-left, width - fullRectWidth)
                    );
                }
                ...
                if (left + width + scrollBarWidth > canvasWidth) {
                    optionAlign.align(
                        positions.RIGHT,
                        Math.min(
                            left + width + scrollBarWidth - canvasWidth,
                            width - fullRectWidth
                        )
                    );
                }
        ...
        watchPostEffect(async () => {
            ...
            const { width: labelWidth } =
                labelRef.value.getBoundingClientRect();
...
.canvas-rect {
    ...
    .hover-line {
        ...
        &.forbidden.in {
            background: var(
                --te-canvas-container-hover-line-in-forbid-bg-color
            );
        }
    }
    ...
    &.multi-select {
        ...
        &.dragging {
            opacity: 0.7;
            border-color: var(
                --te-canvas-container-border-color-multi,
                #1890ff
            );
            border-style: dashed;
            border-width: 2px;
