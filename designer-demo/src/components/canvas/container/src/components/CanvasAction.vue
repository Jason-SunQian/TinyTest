<template>
    ...
    <div v-if="showAction" ref="optionRef" class="corner-mark-right" :style="fixStyle">
      <template v-if="!isModal">
        <div :title="selectParentTitle">
          <icon-chevron-left v-if="showToParent" class="svg-currentcolor" @click.stop="selectParent"></icon-chevron-left>
        </div>
  ...
  <div v-show="hoverState.height && hoverState.width" class="canvas-rect hover">
    ...
    <div v-show="hoverState.configure?.isContainer" class="corner-mark-bottom-right">{{ dragToContainerText }}</div>
  </div>
...
export default {
  ...
  setup(props) {
    ...
    const quickSettingsTitle = computed(() => t('designer.canvas.quickSettings'))
    const selectParentTitle = computed(() => t('designer.canvas.selectParent'))
    ...
    const dragToContainerText = computed(() => t('designer.canvas.dragToContainer'))
