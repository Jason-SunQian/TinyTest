...
<!-- eslint-disable vue/no-root-v-if, vue/max-lines-per-block, vue/block-lang, vue/component-api-style, vue/require-default-prop, eqeqeq, vue/html-self-closing, import/order, vue/require-typed-object-prop -->
<template>
    <div
        v-if="state.visible"
        ...
        const handleResize = (event, type) => {
            ...
            if (state.direction === 'horizontal') {
                ...
                const parentWidth = parseInt(
                    window.getComputedStyle(parentDomNode).width,
                    10
                );
            ...
            if (state.direction === 'vertical') {
                const target =
                    schema.componentName === 'CanvasRow' ? schema : parent;
        ...
        watch(
            ...
            selectState => {
                ...
                if (componentName === 'CanvasCol') {
                    state.direction =
                        parent.children.length > 1 ? 'horizontal' : 'vertical';
                    if (state.direction === 'vertical') {
                        ...
                        const extraWidth = isFirstChild(parent, schema)
                            ? width
                            : 0;
                        state.top = `${top}px`;
