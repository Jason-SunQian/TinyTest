...
<template>
    <div>
        <div class="header">
            <icon-setting class="icon-setting" />
            <div>{{ propertySettingsTitle }}</div>
            <span class="icon-wrap">
                <icon-close class="icon-close" @click="closePanel" />
            </span>
        </div>
...
import { ConfigGroup, ConfigItem } from '@/components/i18n-wrappers';
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const propertySettingsTitle = computed(() =>
            t('designer.canvas.propertySettings')
        );
