<!-- eslint-disable vue/html-self-closing, vue/padding-line-between-blocks, import/order, vue/require-typed-ref, @typescript-eslint/prefer-destructuring -->
<template>
...
export default {
    setup() {
        const sizeStyle = computed(() => {
            const { width, height, maxWidth, minWidth } =
                useLayout().getDimension();
            return {
                width,
                height,
                maxWidth,
        ...
        const calculateSize = ({ movementX }) => {
            ...
            useLayout().setDimension({
                width: `${parseInt(
                    Math.min(
                        Math.max(newWidth, parseInt(minWidth)),
                        parseInt(maxWidth)
                    ),
                    10
                )}px`
        ...
        const setScale = () => {
            ...
            nextTick(() => {
                ...
                const rate =
                    canvasWrap.offsetWidth / resizeDom.value.offsetWidth;
                useLayout().setDimension({
        ...
        const draggablePanel = ref<string | null>(null);
        watch(
            () => useLayout().getPluginWidth(draggablePanel.value),
            setScale,
            { flush: 'post' }
        );

        watch(() => useLayout().getDimension().width, setScale, {
            flush: 'post',
            immediate: true
        });

        watch(() => useLayout().leftFixedPanelsStorage.value, setScale, {
            flush: 'post'
        });
        ...
        watch(
            ...
            value => {
                const currentFixed = useLayout()
                    .getPluginState()
                    .fixedPanels.includes(value);
                const isPanelResizable =
                    useLayout().isPanelWidthResizable(value);
        ...
        watch(() => useLayout().rightFixedPanelsStorage.value, setScale, {
            flush: 'post'
        });
        ...
        watch(
            ...
            value => {
                const currentFixed =
                    useLayout().rightFixedPanelsStorage.value.includes(value);
