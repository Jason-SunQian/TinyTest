...
/** MpCountryInput：v-model + 拖拽即创建 state.mpCountryInputN */
export function handleMpCountryInputExternalDrop(
    insertData: Node,
    ...
    if (
        ...
        const m = /^this\.state\.(mpCountryInput\d+)$/.exec(expr);
        const [, matchedKey = ''] = m || [];
        if (matchedKey) stateKey = matchedKey;
    }

    if (!stateKey) {
        stateKey = allocateIndexedStateKey(stateObj, 'mpCountryInput');
    }
