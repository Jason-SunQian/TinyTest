...
/** MpMobileInput：v-model + 拖拽即创建 state.mpMobileInputN */
export function handleMpMobileInputExternalDrop(
    insertData: Node,
    ...
    if (
        ...
        const m = /^this\.state\.(mpMobileInput\d+)$/.exec(expr);
        const [, matchedKey = ''] = m || [];
        if (matchedKey) stateKey = matchedKey;
    }

    if (!stateKey) {
        stateKey = allocateIndexedStateKey(stateObj, 'mpMobileInput');
    }
