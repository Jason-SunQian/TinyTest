...
/** MpCityInput：v-model + 拖拽即创建 state.mpCityInputN */
export function handleMpCityInputExternalDrop(
    insertData: Node,
    ...
    if (
        ...
        const m = /^this\.state\.(mpCityInput\d+)$/.exec(expr);
        const [, matchedKey = ''] = m || [];
        if (matchedKey) stateKey = matchedKey;
    }

    if (!stateKey) {
        stateKey = allocateIndexedStateKey(stateObj, 'mpCityInput');
    }
