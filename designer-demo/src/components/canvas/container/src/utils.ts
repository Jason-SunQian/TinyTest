 ...
 */
const setClipboardSchema = (event, nodeStr) => {
...
const translateStringToSchema = clipText => {
    ...
    try {
        ...
        if (
            ...
            parsedData.every(item => item?.componentName)
        ) {
            return parsedData;
 ...
 * @return 节点的schema对象
 */
const getClipboardSchema = event =>
    translateStringToSchema(event.clipboardData.getData('text/plain'));

export { setClipboardSchema, getClipboardSchema };
