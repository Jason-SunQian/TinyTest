...
const addScript = (src: string, doc = document): Promise<unknown> => {
...
const addStyle = (href: string, doc = document): Promise<unknown> => {
    return new Promise((resolve, reject) => {
...
const copyObject = (node: unknown): unknown => {
 ...
 * @returns {Promise<unknown>} 返回组件库模块
 */
const dynamicImportComponentLib = async ({
    pkg,
    script
}: DynamicImportParams): Promise<unknown> => {
    ...
    // 这里要取父窗口的地址，因为在iframe中href是about:srcdoc
    const href = window.parent.location.href || location.href;
    const scriptUrl = script.startsWith('.')
...
const getComponents = async ({
    ...
    script,
    components
}: ComponentDependency): Promise<void> => {
    if (!pkg) return;
        ...
        if (!window.TinyLowcodeComponent[componentId]) {
            ...
            if (typeof item === 'string') {
                ...
                window.TinyLowcodeComponent[componentId] =
                    config?.destructuring && config?.exportName
                        ? modules[config.exportName]
                        : modules?.default;
            }
        }
    });
...
export { addScript, addStyle, copyObject, getComponents };
