<template>
  <slot>
    <span :class="['icon', { 'icon-bind': modelValue?.variable }]" @click="open">
      <svg-button name="cloud-shell" placement="top" :tips="t('designer.configurators.variableBinding.title')"></svg-button>
    </span>
  </slot>
  ...
  >
    <div class="bind-dialog-container">
      <div class="bind-dialog-container-header">
        <tiny-alert
          type="info"
          :description="t('designer.configurators.variableBinding.description')"
          class="header-alert"
          :closable="false"
        ></tiny-alert>
      </div>
      <div class="bind-dialog-content">
        <div class="content-left">
          <span class="content-left__title">{{ t('designer.configurators.variableBinding.variableList') }}</span>
          <div class="list-wrap">
            ...
            <div class="item-content">
              <tiny-search v-model="state.value" :placeholder="t('designer.configurators.variableBinding.search')"></tiny-search>
        ...
        <div class="content-right">
          <div class="content-detail-header">
            <div class="header-name">
              <span class="content-right__title">{{ t('designer.configurators.variableBinding.variable') }}</span>
            </div>
          </div>
          <div class="content-wrap">
            ...
            <div v-else :class="['top', { 'poll-top': isDataSource }]">
              ...
              <div v-if="isDataSource" class="datasource-poll-wrap">
                <tiny-tooltip
                  placement="top"
                  :content="t('designer.configurators.variableBinding.pollTooltip')"
                  effect="light"
                  ><span>{{ t('designer.configurators.variableBinding.pollUpdate') }}</span></tiny-tooltip
                >
                <tiny-switch v-model="state.isPoll"></tiny-switch>
                <div v-if="state.isPoll" class="datasource-poll-interval">
                  <span>{{ t('designer.configurators.variableBinding.updateInterval') }}</span>
            ...
            <div class="bottom lowcode-scrollbar-thin">
              <span class="bottom-title">{{ t('designer.configurators.variableBinding.usage') }}</span>
              <div class="bottom-demo">
                <p>
                  {{ t('designer.configurators.variableBinding.usageDescription1') }}
                </p>
                <p>{{ t('designer.configurators.variableBinding.usageDescription2') }}</p>
                <div>{{ t('designer.configurators.variableBinding.usageExample.pageState') }}</div>
                <div>{{ t('designer.configurators.variableBinding.usageExample.string') }}</div>
                <div>{{ t('designer.configurators.variableBinding.usageExample.number') }}</div>
                <div>{{ t('designer.configurators.variableBinding.usageExample.boolean') }}</div>
                <div>{{ t('designer.configurators.variableBinding.usageExample.object') }}</div>
                <div>{{ t('designer.configurators.variableBinding.usageExample.array') }}</div>
                <div>{{ t('designer.configurators.variableBinding.usageExample.null') }}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <template #footer>
      <div class="bind-dialog-footer">
        <tiny-button type="danger" plain @click="remove">{{ t('designer.configurators.variableBinding.removeBinding') }}</tiny-button>
        <div class="right">
          <tiny-button @click="cancel">{{ t('designer.common.cancel') }}</tiny-button>
          <tiny-button type="info" @click="confirm">{{ t('designer.common.confirm') }}</tiny-button>
        </div>
      </div>
    </template>
  </tiny-dialog-box>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // 使用 computed 确保响应式更新
    const list = computed(() => [
      { id: 'state', content: t('designer.configurators.variableBinding.categories.state') },
      { id: 'store', content: t('designer.configurators.variableBinding.categories.store') },
      { id: 'function', content: t('designer.configurators.variableBinding.categories.function') },
      { id: 'utils', content: t('designer.configurators.variableBinding.categories.utils') },
      { id: 'bridge', content: t('designer.configurators.variableBinding.categories.bridge') },
      { id: 'datasource', content: t('designer.configurators.variableBinding.categories.datasource') }
    ])
    ...
    const state = reactive({
      ...
      variableList: computed(() => {
        ...
        if (state.loopData) {
          extendedVars.push({ id: 'loop', content: t('designer.configurators.variableBinding.categories.loop') })
        }

        if (isInJsSlot) {
          extendedVars.push({ id: 'slotScope', content: t('designer.configurators.variableBinding.categories.slotScope') })
        }
