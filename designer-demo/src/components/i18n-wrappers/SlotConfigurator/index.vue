<!-- eslint-disable vue/max-lines-per-block, vue/block-lang, @typescript-eslint/naming-convention, vue/component-api-style, @typescript-eslint/no-shadow, @typescript-eslint/no-empty-function, vue/html-self-closing, import/order, vue/require-typed-object-prop, prefer-const -->
<template>
    <div>
        <div
    ...
    setup(props) {
        ...
        watchEffect(() => {
            ...
            if (Array.isArray(children)) {
                children.forEach(child => {
                    if (
                        child.componentName === 'Template' &&
                        child.props?.slot
                    ) {
                        const slotName =
                            child.props.slot?.name || child.props.slot;
            ...
            if (slot.label) {
                ...
                if (text) {
                    ...
                    if (currentLang === 'en_US' && containsChinese(text)) {
                        ...
                        return (
                            formatPropertyName(slot.name, currentLang) ||
                            slot.name
                        );
                    }
                    return text;
        ...
        const getSlotDescription = slot => {
            ...
            if (slot.description !== undefined && slot.description !== null) {
                // 先尝试获取当前语言的翻译
                const desc = getLocalizedText(slot.description, currentLang);

                // 如果当前是英文但没有英文翻译，尝试使用映射表翻译中文
                if (
                    !desc &&
                    currentLang === 'en_US' &&
                    typeof slot.description === 'object' &&
                    slot.description.zh_CN
                ) {
                    // 尝试使用映射表翻译
                    const translated = translateChinesePropertyName(
                        slot.description.zh_CN
        ...
        const toggleSlot = ({ name = 'default', params, bind }, i) => {
            ...
            if (isPopper) {
                const showProp =
                    typeof isPopper === 'string' ? isPopper : 'modelValue';
