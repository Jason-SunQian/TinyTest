...
export default {
  ...
  setup(props: any, { attrs, emit }) {
    ...
    const i18nButtonText = computed(() => {
      ...
      // 如果已经是多语言对象，直接返回
      if (buttonText && typeof buttonText === 'object' && (buttonText.zh_CN || buttonText.en_US)) {
        return buttonText;
      }
      
      // 如果是字符串且是中文，返回多语言对象
      if (typeof buttonText === 'string' && /[\u4e00-\u9fa5]/.test(buttonText)) {
        return {
          zh_CN: buttonText,
    ...
    const i18nTitle = computed(() => {
      ...
      // 如果已经是多语言对象，直接返回
      if (title && typeof title === 'object' && (title.zh_CN || title.en_US)) {
        return title;
      }
