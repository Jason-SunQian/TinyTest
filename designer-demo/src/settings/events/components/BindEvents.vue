  ...
  <div class="bind-action-list">
    <div class="popover-head">
      ...
      >
        <template #reference>
          <tiny-button class="bind-event-btn">
            <span>{{ t('designer.settings.events.bind.bindEvent') }}</span>
        ...
        <ul class="bind-event-list">
          ...
          >
            <div>{{ name }}&nbsp; | &nbsp;{{ event?.label?.[localeKey] || name }}</div>
          </li>
        </ul>
      </tiny-popover>
      <tiny-button
      ...
      >
        ...
        <span class="custom-event-button-text">{{ t('designer.settings.events.bind.addEvent') }}</span>
      </tiny-button>
    </div>
      ...
      <li v-for="action in state.bindActions" :key="action.eventName">
        <div class="action-item bind-action-item">
          <div class="binding-name" @click="openActionDialog(action)">
            <div>
              {{ action.eventName }}<span>{{ renderEventList[action.eventName]?.label?.[localeKey] }}</span>
            </div>
          ...
          <div class="action-buttons">
            ...
            <svg-button
              ...
              :tips="t('designer.settings.events.bind.locateCode')"
              placement="top"
              :hoverBgColor="false"
    ...
    <div v-show="isEmpty" class="empty-action">
      ...
      <div class="center">{{ t('designer.settings.events.bind.emptyTip') }}</div>
    </div>
  </div>
  ...
  setup() {
    ...
    const delEvent = (action) => {
      confirm({
        ...
        message: t('designer.settings.events.bind.deleteConfirm', { name: action.eventName }),
        exec() {
          if (pageState.isBlock) {
    ...
    const handleAddEvent = (params) => {
      ...
      Object.assign(state.customEvents, {
        [eventName]: {
          ...
          description: {
            zh_CN: t('designer.settings.events.bind.customEventDescription', { name: eventDescription }),
            en_US: t('designer.settings.events.bind.customEventDescription', { name: eventDescription })
          },
          type: 'event',
          functionInfo: {
            params: [],
