<template>
  <div class="content-right">
    <div :class="['content-right-top', { 'tip-error': context.tipError }]">
      <div class="content-right-title">{{ t('designer.settings.events.dialog.methodName') }}</div>
      <tiny-input
        ...
        :placeholder="t('designer.settings.events.dialog.methodNamePlaceholder')"
    ...
    <div :class="['content-right-bottom', { 'tip-error': !context.isValidParams }]">
      <div class="content-right-title">
        <tiny-checkbox v-model="context.enableExtraParams" name="tiny-checkbox">{{ t('designer.settings.events.dialog.extraParams') }}</tiny-checkbox>
        <div class="set-params-tip">
          <div>{{ t('designer.settings.events.dialog.extraParamsTip') }}</div>
          {{ t('designer.settings.events.dialog.extraParamsExample', { name: context.bindMethodInfo.name }) }}
        </div>
      </div>
...
export default {
  ...
  setup() {
    ...
    const change = (value) => {
      const validRules = [
        { validator: validMethodNameEmpty, tip: t(METHOD_TIPS_MAP.empty) },
        { validator: validMethodNameExist, tip: t(METHOD_TIPS_MAP.exist) },
        { validator: invalidMethodName, tip: t(METHOD_TIPS_MAP.ruleInvalid) }
      ]
