  ...
  >
    ...
    <template #footer>
      <div class="bind-dialog-footer">
        <tiny-button @click="closeDialog">{{ t('designer.settings.events.dialog.cancel') }}</tiny-button>
        <tiny-button type="info" @click="confirm">{{ t('designer.settings.events.dialog.confirm') }}</tiny-button>
      </div>
    </template>
  </tiny-dialog-box>
</template>
...
export default {
  ...
  setup(props) {
    const { BindEventsDialogSidebar, BindEventsDialogContent } = getMergeMeta(META_ID).components
