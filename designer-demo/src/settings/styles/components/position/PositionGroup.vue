...
<template>
    <div class="position-group">
        <div
            :class="[
                'position-label',
                { 'is-setting': getSettingFlag(POSITION_PROPERTY.Position) }
            ]"
            :title="t('designer.settings.styles.position.title')"
        ...
        <div v-if="state.showSetting" class="position-dropdown">
                        ...
                        <g>
                            <g>
                                <path
                                    ...
                                    :aria-label="
                                        t(
                                            'designer.settings.styles.position.ariaLabels.top'
                                        )
                                    "
                                    class="tb-path-color"
                                />
                        ...
                        <g>
                            <g>
                                <path
                                    ...
                                    :aria-label="
                                        t(
                                            'designer.settings.styles.position.ariaLabels.right'
                                        )
                                    "
                                    class="lr-path-color"
                                />
                        ...
                        <g>
                            <g>
                                <path
                                    ...
                                    :aria-label="
                                        t(
                                            'designer.settings.styles.position.ariaLabels.bottom'
                                        )
                                    "
                                    class="tb-path-color"
                                />
                        ...
                        <g>
                            <g>
                                <path
                                    ...
                                    :aria-label="
                                        t(
                                            'designer.settings.styles.position.ariaLabels.left'
                                        )
                                    "
                                    class="lr-path-color"
                                />
                    ...
                    <div
                        :class="[
                            ...
                            'top',
                            {
                                ...
                                'is-show':
                                    state.show &&
                                    state.className === POSITION_PROPERTY.Top
                            }
                        ]"
                        @click="clickDirection(POSITION_PROPERTY.Top, $event)"
                    >
                        <span>{{
                            getDirectionText(POSITION_PROPERTY.Top)
                        }}</span>
                    </div>
                    <div
                        :class="[
                            ...
                            {
                                'is-setting': getSettingFlag(
                                    POSITION_PROPERTY.Right
                                ),
                                'is-show':
                                    state.show &&
                                    state.className === POSITION_PROPERTY.Right
                            }
                        ]"
                        @click="clickDirection(POSITION_PROPERTY.Right, $event)"
                    >
                        <span>{{
                            getDirectionText(POSITION_PROPERTY.Right)
                        }}</span>
                    </div>
                    <div
                        :class="[
                            ...
                            {
                                'is-setting': getSettingFlag(
                                    POSITION_PROPERTY.Bottom
                                ),
                                'is-show':
                                    state.show &&
                                    state.className === POSITION_PROPERTY.Bottom
                            }
                        ]"
                        @click="
                            clickDirection(POSITION_PROPERTY.Bottom, $event)
                        "
                    >
                        <span>{{
                            getDirectionText(POSITION_PROPERTY.Bottom)
                        }}</span>
                    </div>
                    <div
                        :class="[
                            ...
                            'left',
                            {
                                ...
                                'is-show':
                                    state.show &&
                                    state.className === POSITION_PROPERTY.Left
                            }
                        ]"
                        @click="clickDirection(POSITION_PROPERTY.Left, $event)"
                    >
                        <span>{{
                            getDirectionText(POSITION_PROPERTY.Left)
                        }}</span>
                    </div>
                </div>
            </div>
            <div class="relative-wrap">
                ...
                <div class="relative">
                    <span>{{
                        t('designer.settings.styles.position.relativeTo')
                    }}</span>
                </div>
                <numeric-select
                    :name="getProperty(POSITION_PROPERTY.ZIndex).name"
                    :numerical-text="getProperty(POSITION_PROPERTY.ZIndex).text"
                    suffix="-"
                    @update="updateStyle"
                />
                <span
                    :class="[
                        ...
                        {
                            'is-setting': getSettingFlag(
                                POSITION_PROPERTY.ZIndex
                            )
                        }
                    ]"
                    @click="openSetting(POSITION_PROPERTY.ZIndex, $event)"
                    >{{ t('designer.settings.styles.position.zIndex') }}</span>
            </div>
        </div>
    </div>
    ...
    <modal-mask v-if="state.showDirectionModal" @close="closeDirectionModal">
        <spacing-setting :property="state.property" @update="update" />
    </modal-mask>
</template>
    ...
    setup(props, { emit }) {
        ...
        const getRelativeToLabel = position =>
            position === 'relative'
                ...
                : t(
                      'designer.settings.styles.position.relativeTargets.container'
                  );
        ...
        const getDirectionText = name =>
            state.directionText[name] || getPropertyText(name) || 'auto';
