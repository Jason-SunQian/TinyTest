...
<template>
    <div class="effect-group line opacity-wrap">
        <label
            :class="[
                'opacity-label',
                { 'is-setting': getSettingFlag(EFFECTS_PROPERTY.Opacity) }
            ]"
            @click="openSetting(EFFECTS_PROPERTY.Opacity, $event)"
        >
            <span>{{ t('designer.settings.styles.effects.opacity') }}</span>
        </label>
        <div>
            <slider-configurator
                :model-value="state.opacity"
                :controls="false"
                :show-unit="true"
    ...
    <div class="effect-group line outline-wrap">
        <label
            :class="[
                'outline-label',
                { 'is-setting': getSettingFlag(EFFECTS_PROPERTY.Outline) }
            ]"
            @click="openSetting(EFFECTS_PROPERTY.Outline, $event)"
        >
            <span>{{ t('designer.settings.styles.effects.outline') }}</span>
        </label>
        <div class="outline-content">
            <tabs-group-configurator
                :options="outlineOptions"
                :model-value="state.activedType"
                label-width="52"
                effect="light"
                placement="top"
                @update:model-value="selectOutlineStyle"
            />
        </div>
        <div
            v-if="state.activedType && state.activedType !== 'none'"
            class="outline-setting"
        >
            <div class="outline-row-wrap">
                <div class="left outline-width">
                    <label class="outline-label"><span>{{
                            t('designer.settings.styles.effects.width')
                        }}</span></label>
                    <input-select
                        :model-value="state.outlineWidth"
                        :suffix-value="state.outlineWidthSuffix"
                ...
                <div class="right outline-width">
                    <label class="outline-label"><span>{{
                            t('designer.settings.styles.effects.offset')
                        }}</span></label>
                    <input-select
                        :model-value="state.outlineOffset"
                        :suffix-value="state.outlineOffsetSuffix"
                        :options="outlineSuffixOptions"
            ...
            <div class="outline-color">
                <label class="outline-label"><span>{{
                        t('designer.settings.styles.effects.color')
                    }}</span></label>
                <color-configurator
                    :model-value="state.outlineColor"
    ...
    <div class="effect-group line cursor-wrap">
        <label
            :class="[
                'cursor-label',
                { 'is-setting': getSettingFlag(EFFECTS_PROPERTY.Cursor) }
            ]"
            @click="openSetting(EFFECTS_PROPERTY.Cursor, $event)"
        >
            <span>{{ t('designer.settings.styles.effects.cursor') }}</span>
        </label>
        <div>
            <tiny-select
                v-model="state.cursorValue"
                :placeholder="
                    t('designer.settings.styles.effects.selectPlaceholder')
                "
                @change="cursorChange"
            >
                <tiny-option-group
                ...
                >
                    ...
                    >
                        <div class="cursor-option">
                            <span class="item-icon"><svg-icon :name="item.icon"/></span>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const outlineOptions = computed(() =>
            outlineOptionDefs.map(({ key, ...rest }) => ({
                ...rest,
                content: t(`designer.settings.styles.effects.${key}`)
            }))
        );
        ...
        const openSetting = (name, event) => {
            if (getSettingFlag(name)) {
                if (name === 'outline') {
                    activedName = [
                        EFFECTS_PROPERTY.Outline,
                        EFFECTS_PROPERTY.OutlineOffset
                    ];
                } else {
                    activedName = [name];
                }
        ...
        const setOutlineStyle = () => {
            updateStyle({
                [EFFECTS_PROPERTY.Outline]: `${
                    state.outlineWidth + state.outlineWidthSuffix
                } ${state.activedType} ${state.outlineColor}`
            });
        };
        ...
        const setOutlineOffset = () => {
            updateStyle({
                [EFFECTS_PROPERTY.OutlineOffset]: `${
                    state.outlineOffset + state.outlineOffsetSuffix
                }`
            });
        };
        ...
        const selectOutlineStyle = type => {
            ...
            if (type === 'none') {
                updateStyle({
                    [EFFECTS_PROPERTY.Outline]: null,
                    [EFFECTS_PROPERTY.OutlineOffset]: null
                });
            } else {
                setOutlineStyle();
                setOutlineOffset();
