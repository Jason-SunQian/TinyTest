...
<template>
    <div
        :class="[
            'content-wrap',
            { 'is-margin': isMargin, 'is-padding': isPadding }
        ]"
    >
        <div class="content-input">
            <span class="icon">
                <svg-icon v-show="isMargin" name="margin" />
                <svg-icon v-show="isPadding" name="padding" />
            </span>
            <numeric-select
                :name="property.name"
                :min="minVal"
                :numerical-text="numericalText"
                @update="inputChange"
            />
        </div>
        <div class="content-value">
            <div
                v-show="isMargin"
                :class="['auto', { active: 'auto' === propertyValue }]"
                @click="select('auto')"
            >
                Auto
            </div>
...
<!-- eslint-disable vue/block-lang, vue/component-api-style, @typescript-eslint/naming-convention, no-param-reassign, vue/require-typed-object-prop -->
<script lang="ts">
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const isMargin = computed(
            () =>
                props.property.type === SPACING_PROPERTY.Margin ||
                props.property.type === POSITION_PROPERTY.Position
        );
        const isPadding = computed(
            () => props.property.type === SPACING_PROPERTY.Padding
        );

        const propertyValue = computed(() =>
            props.property.value === 'auto'
                ? 'auto'
                : Number.parseInt(props.property.value)
        );
        const sliderValue = computed(() =>
            Number.parseInt(props.property.value || 0)
        );

        const numericalText = computed(() =>
            props.property.value?.indexOf('px') > -1
                ? Number.parseInt(props.property.value)
        ...
        .custom {
            ...
            .option {
                ...
                &.active {
                    color: var(--te-styles-common-text-color-primary);
                    background: var(
                        --te-styles-spacing-setting-bg-color-active
                    );
                }
            }
        }
