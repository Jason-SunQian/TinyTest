...
<template>
    <div class="className-container">
        ...
        <div class="selector-container">
            ...
            >
                <div
                    class="edit-global-css"
                    :title="
                        t('designer.settings.styles.globalStyle.editButton')
                    "
                    @click="scope.open"
                >
            ...
            <div class="selector-right-container">
                <div class="selector-right-container-wrap">
                    <div
                        :class="[
                            ...
                            { 'has-error': classNameState.selectorHasError }
                        ]"
                    ...
                    >
                        <div
                            v-if="
                                ...
                                classNameState.curSelectorIsEditing
                            "
                        ...
                        >
                            <div class="current-selector-label">
                                ...
                                <div
                                    v-if="
                                        !classNameState.curSelectorIsEditing &&
                                        classNameState.curSelectorEditable
                                    "
                                    class="edit-wrap"
                                >
                                    <svg-icon
                                        ...
                                        :title="
                                            t(
                                                'designer.settings.styles.globalStyle.actions.edit'
                                            )
                                        "
                                        class="edit-btn"
                                        @click.stop="handleEditCurSelector"
                                    ...
                                    <svg-icon
                                        ...
                                        :title="
                                            t(
                                                'designer.settings.styles.globalStyle.actions.delete'
                                            )
                                        "
                                        class="delete-btn"
                        ...
                        <span v-else class="empty-tips">{{
                            t(
                                'designer.settings.styles.globalStyle.placeholder'
                            )
                        }}</span>
                        <input
                    ...
                    <tiny-select
                        v-model="state.className.mouseState"
                ...
                <div v-if="classNameState.selectorHasError" class="error-tips">
                    <svg-icon name="error" />
                    <span class="error-tips-text">{{
                        classNameState.selectorHasError
                    }}</span>
                </div>
                <div
                    v-if="classNameState.showDropdownList"
                    class="selector-drop-down-list lowcode-scrollbar-thin"
                >
                    <span class="selector-dropdown-list-tips">
                        {{
                            t(
                                'designer.settings.styles.globalStyle.dropdown.create'
                            )
                        }}
                    </span>
                    <span
                        v-if="currentSelectorList.length"
                        class="selector-dropdown-list-tips"
                    >
                        {{
                            t(
                                'designer.settings.styles.globalStyle.dropdown.selectExisting'
                            )
                        }}
                    </span>
                    ...
                    <span
                        v-if="state.selectors.length"
                        class="selector-dropdown-list-tips add-global-class-tips"
                    >
                        {{
                            t(
                                'designer.settings.styles.globalStyle.dropdown.addToComponent'
                            )
                        }}
                    </span>
                    <ul class="exist-class-list">
...
const docsContent = computed(() =>
    t('designer.settings.styles.globalStyle.docsTip')
);
...
const setSelectorError = (key = '') => {
    ...
    classNameState.selectorHasError = key
        ? t(`designer.settings.styles.globalStyle.errors.${key}`)
        : '';
};

watch(locale, () => {
    if (classNameState.selectorErrorKey) {
        classNameState.selectorHasError = t(
            `designer.settings.styles.globalStyle.errors.${classNameState.selectorErrorKey}`
        );
    }
});
...
const errorKeyMap = {
    ...
    singleSelector: t(
        'designer.settings.styles.globalStyle.errors.singleSelector'
    ),
...
const editClassName = (
    ...
    const editSelectorHandler = () => {
        const oldSelType = oldSelector.startsWith('.')
    ...
    const removeSelectorHandler = () => {
        const leftSelectors =
            typeMap[type]
                .split(' ')
                .filter(item => item !== newClassNames && Boolean(item)) || [];
...
const handleCreateNewClass = () => {
    ...
    let { newSelector } = classNameState;
...
const handleEditCurSelector = async () => {
    ...
    range.setEnd(
        selectorTextRef.value.childNodes[0],
        selectorTextRef.value.textContent.length
    );
...
.selector-container {
    ...
    .className-selector-container {
        ...
        &:hover {
            border-color: var(
                --te-styles-className-selector-container-border-color-hover
            );
        }
        &.has-error {
            border-color: var(
                --te-styles-className-selector-container-border-color-error
            );
            background-color: var(
                --te-styles-className-selector-container-bg-color-error
            );
        }
        ...
        .empty-tips {
            position: absolute;
            color: var(
                --te-styles-className-selector-container-empty-tips-text-color
            );
            z-index: 0;
        }
        .current-selector {
            ...
            .current-selector-label {
                ...
                .delete-btn {
                    color: var(
                        --te-styles-className-selector-container-option-btn-text-color
                    );
                    margin-left: 4px;
                    cursor: pointer;
                }
    ...
    .error-tips {
        ...
        font-size: 12px;
        color: var(
            --te-styles-className-selector-container-tips-text-color-error
        );
        .error-tips-text {
            margin-left: 4px;
        }
    ...
    .selector-drop-down-list {
        ...
        padding: 8px 0;
        background-color: var(
            --te-styles-className-selector-container-dropdown-list-bg-color
        );
        ...
        .exist-class-item {
            ...
            &:hover {
                background-color: var(
                    --te-styles-className-selector-container-dropdown-list-item-bg-color-active
                );
                color: var(--te-styles-common-text-color-primary);
            }
        }
