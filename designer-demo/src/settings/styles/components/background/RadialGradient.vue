<!-- eslint-disable vue/no-bare-strings-in-template, vue/block-lang, @typescript-eslint/no-empty-function, no-inline-comments, line-comment-position, @typescript-eslint/prefer-destructuring, vue/define-props-declaration, vue/require-typed-object-prop, vue/define-emits-declaration, vue/max-lines-per-block -->
<template>
    <position-origin
        class="background-row line"
        :is-top="true"
        @update="updatePositionOrigin"
    />
    <div class="background-row line">
        <label class="row-label">Size</label>
        <tabs-group-configurator
            :options="RADIAL_SIZE_LIST"
            :model-value="state.radialSize"
            label-width="50"
            :effect="effect"
            :placement="placement"
            @update:model-value="selectRadialSize"
        />
    </div>
...
const updateStyle = () => {
    const property = {
        [BACKGROUND_PROPERTY.BackgroundImage]: `${
            state.repeat ? 'repeating-' : ''
        }radial-gradient(circle ${state.radialSize} at ${
            state[[BACKGROUND_PROPERTY.BackgroundPosition]]
        }, ${state.colorList.join(',')})`,
...
onMounted(() => {
    ...
    // closest-side
    const rExtentKeyword = /([-a-z]+)?/;
    // 50% 50%
    const rPosition = /(\d+%(?:\s)\d+%)/;
    const rGradientCapture = new RegExp(
        `circle ${rExtentKeyword.source}\\s*at ${rPosition.source},`,
        'gi'
    );
    const gradientMatch = rGradientCapture.exec(
