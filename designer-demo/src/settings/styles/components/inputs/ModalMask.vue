<!-- eslint-disable vue/require-explicit-emits, vue/block-lang, vue/component-api-style, @typescript-eslint/no-shadow, vue/require-typed-ref -->
<template>
    <teleport :to="targetClass">
        <div class="modal-wrapper">
            <div
                :class="[isAlignBody ? '' : 'modal-mask']"
                @click="$emit('close')"
            />
            ...
            <div
                ...
                :class="[
                    'modal-content',
                    { 'align-body': isAlignBody },
                    { 'modal-padding': isAlignBody }
                ]"
            >
                <slot />
            </div>
...
export const useModal = () => {
    const setPosition = event => {
        const panelWidth = window
            .getComputedStyle(document.body)
            .getPropertyValue('--base-right-panel-width');
    ...
    setup(props) {
        ...
        const calculateTopStyle = modalContentParam => {
            ...
            if (isAlignBody && modalContentParam) {
                return modal.top < modalContentParam.offsetHeight
                    ? 40
                    : modal.top > parseInt(innnerHeight) - 400
                    ? modal.top - 364
                    : modal.top - modalContentParam.offsetHeight + 40;
            }
            return modal.top - 34;
        };
...
.modal-wrapper {
    ...
    .modal-padding {
        padding: var(--te-styles-modal-padding-y)
            var(--te-styles-modal-padding-x);
    }
    .align-body {
        right: calc(
            var(--te-styles-modal-right-offset-first) +
                var(--base-nav-panel-width)
        );
        left: calc(
            100% - var(--te-styles-modal-right-offset-first) -
                var(--te-styles-modal-right-offset-second) -
