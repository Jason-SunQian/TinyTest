    ...
    <div class="size-row">
        <div class="size-col">
            <div
                :class="[
                    'size-label',
                    { 'is-setting': getSettingFlag(SIZE_PROPERTY.Width) }
                ]"
                @click="openSetting(SIZE_PROPERTY.Width, $event)"
            >
                <span>{{ t('designer.settings.styles.size.width') }}</span>
            </div>
            <numeric-select
                :name="getProperty(SIZE_PROPERTY.Width).name"
                :numerical-text="getProperty(SIZE_PROPERTY.Width).text"
                @update="updateStyle"
            />
        </div>
        <div class="size-col">
            <div
                :class="[
                    'size-label',
                    { 'is-setting': getSettingFlag(SIZE_PROPERTY.Height) }
                ]"
                @click="openSetting(SIZE_PROPERTY.Height, $event)"
            >
                <span>{{ t('designer.settings.styles.size.height') }}</span>
            </div>
            <numeric-select
                :name="getProperty(SIZE_PROPERTY.Height).name"
                :numerical-text="getProperty(SIZE_PROPERTY.Height).text"
                @update="updateStyle"
            />
        </div>
    </div>
    <div class="size-row">
        <div class="size-col">
            <div
                :class="[
                    'size-label',
                    { 'is-setting': getSettingFlag(SIZE_PROPERTY.MinWidth) }
                ]"
                @click="openSetting(SIZE_PROPERTY.MinWidth, $event)"
            >
                <span>{{ t('designer.settings.styles.size.minWidth') }}</span>
            </div>
            <numeric-select
                :name="getProperty(SIZE_PROPERTY.MinWidth).name"
                :numerical-text="getProperty(SIZE_PROPERTY.MinWidth).text"
                @update="updateStyle"
            />
        </div>
        <div class="size-col">
            <div
                :class="[
                    'size-label',
                    { 'is-setting': getSettingFlag(SIZE_PROPERTY.MinHeight) }
                ]"
                @click="openSetting(SIZE_PROPERTY.MinHeight, $event)"
            >
                <span>{{ t('designer.settings.styles.size.minHeight') }}</span>
            </div>
            <numeric-select
                :name="getProperty(SIZE_PROPERTY.MinHeight).name"
                :numerical-text="getProperty(SIZE_PROPERTY.MinHeight).text"
                @update="updateStyle"
            />
        </div>
    </div>
    <div class="size-row">
        <div class="size-col">
            <div
                :class="[
                    'size-label',
                    { 'is-setting': getSettingFlag(SIZE_PROPERTY.MaxWidth) }
                ]"
                @click="openSetting(SIZE_PROPERTY.MaxWidth, $event)"
            >
                <span>{{ t('designer.settings.styles.size.maxWidth') }}</span>
            </div>
            <numeric-select
                :name="getProperty(SIZE_PROPERTY.MaxWidth).name"
                :numerical-text="getProperty(SIZE_PROPERTY.MaxWidth).text"
                @update="updateStyle"
            />
        </div>
        <div class="size-col">
            <div
                :class="[
                    'size-label',
                    { 'is-setting': getSettingFlag(SIZE_PROPERTY.MaxHeight) }
                ]"
                @click="openSetting(SIZE_PROPERTY.MaxHeight, $event)"
            >
                <span>{{ t('designer.settings.styles.size.maxHeight') }}</span>
            </div>
            <numeric-select
                :name="getProperty(SIZE_PROPERTY.MaxHeight).name"
                :numerical-text="getProperty(SIZE_PROPERTY.MaxHeight).text"
                @update="updateStyle"
            />
        </div>
    </div>
    ...
    <div class="overflow-row">
        <div
            :class="[
                'overflow-label',
                {
                    'is-setting': getSettingFlag(SIZE_PROPERTY.Overflow),
                    selected: Boolean(selectedOverflow)
                }
            ]"
            @click="openSetting(SIZE_PROPERTY.Overflow, $event)"
        >
            <span>{{ t('designer.settings.styles.size.overflow') }}</span>
        </div>
        <tabs-group-configurator
            :options="overflowOptions"
            :model-value="selectedOverflow"
            :label-width="50"
            :effect="effect"
            :placement="placement"
    ...
    <div class="fit-row">
        <div
            :class="[
                'fit-label',
                {
                    'is-setting': getSettingFlag(SIZE_PROPERTY.ObjectFit),
                    selected: state.value !== 'object-fit:fill'
                }
            ]"
            title="object-fit"
            @click="openSetting(SIZE_PROPERTY.ObjectFit, $event)"
        >
            <span>{{ t('designer.settings.styles.size.fit') }}</span>
        </div>
        <div class="fit-select">
            <select-configurator
                :model-value="state.value"
                :options="fitOptions"
                @update:model-value="selectFit"
            />
        </div>
        <tiny-tooltip
            :effect="effect"
            ...
            :placement="placement"
            :content="t('designer.settings.styles.size.objectPositionTooltip')"
        >
            <tiny-popover
                placement="bottom"
            ...
            >
                ...
                <div class="size-fit-content">
                    <div
                        ...
                        @click="
                            openSetting(SIZE_PROPERTY.ObjectPosition, $event)
                        "
                    >
                        {{ t('designer.settings.styles.size.objectPosition') }}
                    </div>
                    <div class="size-fit-content-main">
                        <div class="coordinate">
                            <div
                                ...
                                :class="[
                                    'coordinate-origin',
                                    { selected: state.originActive === index }
                                ]"
                                @click="selectOrigin(item, index)"
                            >
                                <svg-icon
                                    v-if="state.originActive !== index"
                                    :name="item.pointIcon"
                                />
                                <svg-icon v-else :name="item.originIcon" />
                            </div>
                        </div>
...
<!-- eslint-disable vue/max-lines-per-block, vue/block-lang, vue/component-api-style, @typescript-eslint/naming-convention, max-lines, vue/require-typed-object-prop -->
<script lang="ts">
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const fitOptionDefs = [
            { key: 'fill', value: 'object-fit:fill' },
            { key: 'contain', value: 'object-fit:contain' },
            { key: 'cover', value: 'object-fit:cover' },
            { key: 'none', value: 'object-fit:none' },
            { key: 'scaleDown', value: 'object-fit:scale-down' }
        ];

        const fitOptions = computed(() =>
            fitOptionDefs.map(({ key, value }) => ({
                label: t(`designer.settings.styles.fitOptions.${key}`),
                value
            }))
        );
        const originOptions = [
        ...
        const overflowOptionDefs = [
            { key: 'visible', icon: 'eye', value: OVERFLOW_TYPE.Visible },
            {
                key: 'hidden',
                icon: 'eye-invisible',
            ...
            { key: 'auto', icon: 'overflow-scroll', value: OVERFLOW_TYPE.Auto }
        ];

        const overflowOptions = computed(() =>
            overflowOptionDefs.map(({ key, icon, value }) => ({
                icon,
                content: t(`designer.settings.styles.overflowOptions.${key}`),
                value
            }))
        );
