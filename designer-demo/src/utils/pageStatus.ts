...
export const ensureOccupier = (occupier?: Record<string, any>) => {
  const userInfo = getMetaApi(META_SERVICE.GlobalService).getState()?.userInfo
...
export const getEnsuredCanvasStatus = (occupier?: Record<string, any>) => {
  ...
  if (ENABLE_PAGE_LOCK_GUARD && ![PAGE_STATUS.Lock, PAGE_STATUS.Guest].includes(status.state)) {
