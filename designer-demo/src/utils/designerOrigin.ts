...
export function getDesignerMaterialBaseUrl(): string | null {
    ...
    const origin = (win.TINY_DESIGNER_ORIGIN || (import.meta.env as any).VITE_ORIGIN || 'http://localhost:8090').replace(/\/$/, '');
    // 开发模式（ORIGIN 为 localhost）：画布 iframe 内 vscode-webview 会 403，统一用 localhost 加载物料
    if (origin.includes('localhost') || origin.includes('127.0.0.1')) {
        return origin;
    }
    const base =
        win.TINY_DESIGNER_MATERIALS_BASE ||
        origin;
...
export function toAbsoluteMaterialUrl(
    ...
    if (isMaterialsBase && typeof window !== 'undefined') {
        const map = (window as any).TINY_DESIGNER_MATERIALS_MAP as Record<string, string> | undefined;
