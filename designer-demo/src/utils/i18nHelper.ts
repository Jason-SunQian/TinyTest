 ...
 * @returns 当前语言的文本
 */
export function getNestedLocalizedText(
    obj: any,
    currentLocale: string
): string {
    if (!obj) {
 ...
 * 常见中文属性名到英文的映射表
 */
export const CHINESE_PROPERTY_NAME_MAP: Record<string, string> = {
    '前置内容': 'Prefix Content',
    '后置内容': 'Suffix Content',
    '字段名': 'Field',
    '占位符': 'Placeholder',
 ...
 */
export function translateChineseGroupName(zhText: string): string {
 ...
 * @returns 英文翻译，如果没有找到则返回空字符串
 */
export function translateChinesePropertyName(zhText: string): string {
    if (!zhText || typeof zhText !== 'string') {
        return '';
    }
    // 精确匹配
 ...
 */
export function formatPropertyName(str: string, currentLocale?: string): string {
    if (!str || typeof str !== 'string') {
        return str || '';
    }
