  ...
  const replaceText = () => {
    try {
      ...
      if (i18nInstance?.global) {
        ...
        englishText = i18nInstance.global.t('designer.canvas.dragComponentHere') || englishText
        analyzingText = i18nInstance.global.t('designer.canvas.pageAnalyzing') || analyzingText
        // 恢复原语言
      ...
      // 查找 .empty-text 元素
      const emptyTextElements = doc.querySelectorAll('p.empty-text, .empty-text')
  ...
  try {
    ...
    const setupRenderer = (canvasApi: any) => {
      try {
        ...
        const i18nRenderer = (schema: any, refreshKey: any, entry: any, active: any, isPage = true) => {
          // 强制使用英文翻译
          const i18nInstance = (window as any).lowcodeI18n
          ...
          if (i18nInstance?.global) {
            const originalLocale = i18nInstance.global.locale.value
            i18nInstance.global.locale.value = 'en_US'
            currentText = i18nInstance.global.t('designer.canvas.dragComponentHere') || currentText
            i18nInstance.global.locale.value = originalLocale
          }
          ...
          if (!entry) {
            ...
            if (isEmpty) {
              return [h(I18nCanvasEmpty, { placeholderText: currentText })]
            }
            const result = defaultRenderer(schema, refreshKey, entry, active, isPage)
            return replaceCanvasEmptyInVNode(result, I18nCanvasEmpty, currentText)
          } else {
