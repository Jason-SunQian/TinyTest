...
-   **[主工程应用级 state 导入思路](./主工程应用级state导入思路.md)**
    -   模块单例 app state → `this.appState.<key>`（如 mainTabBar）
-   **[主工程公共能力总结和导入记录](./主工程公共能力总结和导入记录.md)**
    -   按类别记录 utils / stores / composables / appState / modal 等导入进度与下一步
...
-   **[主工程低代码区块开发思路](./主工程低代码区块开发思路.md)** ⭐ Block 开发必读

    -   低代码 **Block（区块）** 与 Page 的区别、创建/出码/嵌入流程、POC 固化结论、阶段规划与优化清单
...
-   **[物料面板冷启动与重复展示修复经验](./物料面板冷启动与重复展示修复经验.md)** ⭐ 2026-09 排障必读

    -   Materials 2x/3x、主工程晚到、`runExclusiveColdStart unavailable` 根因分层
    -   MaterialsDiag 健康日志画像、已落地手段、隐藏隐患与改动评估入口
...
-   **Page JS 如何用应用级 state？** → [主工程应用级 state 导入思路](./主工程应用级state导入思路.md)
