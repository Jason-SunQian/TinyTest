...
>
> **2026-09 更新**：面板多倍展示 / 主工程冷启动竞态的**已验证修复与经验**见  
> [物料面板冷启动与重复展示修复经验.md](./物料面板冷启动与重复展示修复经验.md)（含 MaterialsDiag 验收标准与残留隐患评估）。
...
-   **2026-09 进展**：已用 exclusive cold start + ownership 写拦截 + 面板 key/watch 修复为主路径；详见  
    [物料面板冷启动与重复展示修复经验.md](./物料面板冷启动与重复展示修复经验.md)。  
    仍可能残留 store 内 `htmlChildCount` 偏大（UI 已 dedupe），是否继续收敛需单独评估。
...
_文档更新时间：按当前代码还原后的状态记录；多倍面板问题的现行结论见 2026-09 经验文档。_
